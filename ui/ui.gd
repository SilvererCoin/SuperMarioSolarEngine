...
func _input(event: InputEvent):
	if (
		(event.is_action_pressed(&"camera_zoom_in") and camera.target_zoom != camera.camera_properties.zoom_min) or
		(event.is_action_pressed(&"camera_zoom_out") and camera.target_zoom != camera.camera_properties.zoom_max)
	):
